        ...
        <div class="col-lg-12 grid-margin stretch-card">
          <div class="card">
            <div class="card-body">
              ...
              <p class="card-description">Add your data distributions In</p>
              ...
              <div class="forms-sample">
                  ...
                  <div class="col-12 col-md-12">
                    <div class="row">
                      <div class="col-12 col-md-12 col-sm-12 col-xs-12">
                        <label class="">Referrance Type</label>
                        <div class="form-group row">
                          <div class="col-sm-4">
                            <div class="form-check">
                              <label class="form-check-label">
                                <input type="radio" class="form-check-input" name="poOrDo" id="po" v-model="dist_in.type" value="false" :class="{'is-invalid': errors.has('type')}" data-vv-name="type" v-validate data-vv-rules="required" @change="changeType(false)"> Purchase Order Number <i class="input-helper"></i></label>
                            </div>
                          </div>
                          <div class="col-sm-5">
                            <div class="form-check">
                              <label class="form-check-label">
                                <input type="radio" class="form-check-input" name="poOrDo" id="do" v-model="dist_in.type" value="true" :class="{'is-invalid': errors.has('type')}" data-vv-name="type" v-validate data-vv-rules="required" @change="changeType(true)"> Distribution Out Number <i class="input-helper"></i></label>
                            </div>
                          </div>
                          <label id="type-error" class="error mt-2 text-danger" for="type">{{ errors.first('type') }}</label>
                        </div>
                      </div>
                      <div class="col-6 col-md-6" v-if="dist_in.type === false">
                        <div class="form-group">
                          <label for="po_number">Purchase Order Number</label>
                          <select class="form-control p_input" id="po_number" :class="{'is-invalid': errors.has('po_number')}" placeholder="Choose Purchase Order Number Here..." v-model="dist_in.po_number" name="po_number" data-vv-name="po_number" v-validate data-vv-rules="required">
                            <option disabled :value="null">--- Pilih Purchase Order Number ---</option>
                            <option v-for="(sp, ix) in listDataPo" :key="ix" v-bind:value="sp.po_number">{{ sp.po_number }}</option>
                          </select>
                        </div>
                      </div>
                      <div class="col-6 col-md-6" v-else>
                        <div class="form-group">
                          <label for="do_number">Distribution Out Number</label>
                          <select class="form-control p_input" id="do_number" :class="{'is-invalid': errors.has('do_number')}" placeholder="Choose Distribution Out Number Here..." v-model="dist_in.do_number" name="do_number" data-vv-name="do_number" v-validate data-vv-rules="required">
                            <option disabled :value="null">--- Pilih Distribution Out Number ---</option>
                            <option v-for="dot in listDataDo" :key="dot.distribution_out_id" v-bind:value="dot.do_number">{{ dot.do_number }}</option>
                          </select>
                        </div>
                      </div>
                      <div class="col-6 col-md-6">
                        <div class="form-group">
                          ...
                          <select class="form-control p_input" id="branch" :class="{'is-invalid': errors.has('branch')}" placeholder="Choose Your Branch Here..." v-model="dist_in.branch_id" name="branch_id" data-vv-name="branch" v-validate data-vv-rules="required">
                ...
                <div class="row mt-5">
                  <div class="col-12 col-md-12 text-end">
                    <button type="submit" class="btn btn-primary me-2" @click="createDi">Submit</button>
    ...
    export default {
      ...
      methods: {
        ...
        createDi: function (e) {
          ...
          var data = self.dist_in
          data.user_id = self.meh.id
          if (data.type === false) {
            data.type = 0
          } else {
            data.type = 1
          ...
          self.$store.dispatch('saveDistributionIn', {data,
        ...
        addField: function () {
          var lastObject = this.dist_in.products[this.dist_in.products.length - 1]
          var lastId = lastObject.id + 1
          this.dist_in.products.push({
            id: lastId,
            product_id: "",
            pack_id: 1,
            quantity: 0,
        ...
        supplierPicked: function () {
          var self = this
          return self.getOneSupplier(self.dist_in.supplier_id)
        },
        formatPrice(value) {
          let val = (value/1).toFixed(0).replace('.', ',')
          return val.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")
        },
        changeType: function (types) {
          return this.dist_in.type = types
        }
      },
      computed: {
        ...mapGetters([
        ...
        subtotals: function () {
          let calcSubTotal = 0
          this.dist_in.products.forEach(function (data) {
